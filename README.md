...
**[Download for macOS](https://github.com/prabchevski/telegram-search-mcp/archive/refs/heads/main.zip)**

Extract the ZIP and open `install-macos.command`. No manual build is needed.
Follow the [quick start guide](START_HERE.md) to sign in and select your clients.
...
access. The source download above is generated by GitHub from the current `main`
branch and is available without a GitHub account. Versioned archives with checksums
are available under [Releases](https://github.com/prabchevski/telegram-search-mcp/releases).

Each person uses their own Telegram API credentials and signs in on their own Mac.
Share the repository link or a clean source/release archive. Do not share an installed copy together
...
Archive building is for maintainers preparing releases; users can install directly
from the GitHub source ZIP.
